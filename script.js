const STORAGE_KEY = "nova_day_training_upgrade_v4_full";
...
const defaults = {
  ...
  training: buildDefaultTraining()
};

function makeExercise(name, weight = 0, sets = 0, reps = 0, notes = "", done = false) {
  return { id: uid(), name, weight, sets, reps, notes, done };
}
function buildDay(id, name, exercises) {
  return { id, name, exercises: exercises.map(ex => ({ ...ex, id: uid() })) };
}
function buildDefaultTraining() {
  const bbsExercises = [
    makeExercise("Schrägbank Kurzhantel", 26, 2, 0, ""),
    makeExercise("CableFly", 13, 2, 0, ""),
    makeExercise("Dips", 54, 3, 0, ""),
    makeExercise("BizepsCurl Sitzend", 18, 2, 0, ""),
    makeExercise("BackCableCurls", 13, 2, 0, ""),
    makeExercise("Front Delt Machine", 50, 2, 0, ""),
    makeExercise("Cable LR", 6, 0, 0, ""),
    makeExercise("FacePulls", 29, 0, 0, ""),
    makeExercise("Shrugs", 24, 0, 0, "")
  ];
  const legsAbsExercises = [
    makeExercise("Leg Press", 60, 0, 0, ""),
    makeExercise("Leg Extensions", 77, 0, 0, ""),
    makeExercise("L5", 60, 0, 0, ""),
    makeExercise("Wadenheben", 50, 0, 0, ""),
    makeExercise("Abs Crunches", 70, 0, 0, ""),
    makeExercise("Roasty Maschine", 72, 0, 0, ""),
    makeExercise("V UPS", 0, 0, 0, "")
  ];
  const rtExercises = [
    makeExercise("Klimmzüge", 68, 0, 0, ""),
    makeExercise("Latpulldown eng", 68, 0, 0, "Stuhl: 7 • Pin: 4 • Bank: 30°"),
    makeExercise("Einarmiges Rudern", 59, 0, 0, ""),
    makeExercise("Row Machine", 43, 0, 0, "H: 6 • V: 5"),
    makeExercise("Lower Back Machine", 55, 0, 0, "U: 1 • O: 4"),
    makeExercise("Cross Body Triceps", 6, 0, 0, ""),
    makeExercise("Tricep 3eck", 26, 0, 0, ""),
    makeExercise("Sam Sulek Cable", 38, 0, 0, "")
  ];
  const restExercises = [
    makeExercise("Liegestützen", 0, 0, 0, ""),
    makeExercise("Hammer Curls", 0, 0, 0, "")
  ];
  return {
    activeDayId: "day-bbs-1",
    days: [
      buildDay("day-bbs-1", "BBS", bbsExercises),
      buildDay("day-legsabs-1", "LegsAbs", legsAbsExercises),
      buildDay("day-rt-1", "RT", rtExercises),
      buildDay("day-rest-1", "Rest", restExercises),
      buildDay("day-bbs-2", "BBS", bbsExercises),
      buildDay("day-legsabs-2", "LegsAbs", legsAbsExercises),
      buildDay("day-rt-2", "RT", rtExercises)
    ]
  };
}
function isExpectedTrainingPlan(training) {
  if (!training || !Array.isArray(training.days)) return false;
  const expected = ["BBS", "LegsAbs", "RT", "Rest", "BBS", "LegsAbs", "RT"];
  if (training.days.length !== expected.length) return false;
  return expected.every((name, idx) => (training.days[idx] && training.days[idx].name) === name);
}
...
function loadState() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY) || localStorage.getItem("nova_day_training_upgrade_v2") || localStorage.getItem("nova_day_training_upgrade_v1");
    if (!raw) return clone(defaults);
    const parsed = JSON.parse(raw);
    if (!parsed.training || !parsed.training.days || !parsed.training.days.length || !isExpectedTrainingPlan(parsed.training)) {
      parsed.training = clone(defaults.training);
    }
...
function renameTrainingDay(dayId) {
  const day = state.training.days.find(item => item.id === dayId);
  if (!day) return;
  const nextName = prompt("Neuen Namen für den Day eingeben:", day.name || "");
...
function moveTrainingDay(dayId, direction) {
  const idx = state.training.days.findIndex(day => day.id === dayId);
  if (idx < 0) return;
  const nextIdx = idx + direction;
  if (nextIdx < 0 || nextIdx >= state.training.days.length) return;
  const [day] = state.training.days.splice(idx, 1);
  state.training.days.splice(nextIdx, 0, day);
  saveState();
  renderAll();
}
...
function renderTraining() {
  ...
  trainingDaysEl.innerHTML = state.training.days.map((day, idx) => {
    ...
    return `
      <div class="training-day-card ${day.id === activeDay.id ? "active" : ""}" data-training-action="select-day" data-day-id="${day.id}">
        <div class="training-day-top">
          <span>⋮⋮</span>
          <div class="training-day-mini-actions">
            <button type="button" class="day-mini-btn" data-training-action="rename-day" data-day-id="${day.id}">✎</button>
            <button type="button" class="day-mini-btn" data-training-action="move-day-left" data-day-id="${day.id}" ${idx === 0 ? "disabled" : ""}>←</button>
            <button type="button" class="day-mini-btn" data-training-action="move-day-right" data-day-id="${day.id}" ${idx === state.training.days.length - 1 ? "disabled" : ""}>→</button>
            <button type="button" class="day-mini-btn" data-training-action="delete-day" data-day-id="${day.id}" ${state.training.days.length <= 1 ? "disabled" : ""}>✕</button>
          </div>
        </div>
...
document.body.addEventListener("click", e => {
  ...
  if (trainingBtn) {
    ...
    if (action === "rename-day" && dayId) {
      renameTrainingDay(dayId);
      return;
    }
    ...
    if (action === "move-day-left" && dayId) {
      moveTrainingDay(dayId, -1);
      return;
    }
    if (action === "move-day-right" && dayId) {
      moveTrainingDay(dayId, 1);
      return;
    }
